...
class Model():
    ...
    def __init__(self, env, configuration, idle_strategy = 'Ph', time_saving_percent = 20):
        ...
        - env contains the following entities:
            ...
            - The CFS incidents for the shift (env.incidents) -> Incident objects
        - configuration contains the 

        - by default, the idle strategy of all agents is Ph
        """
        ...
        #self.precincts = gpd.read_file('Council_Districts/Council_Districts.shp')
        ...
        # Overall, I'm using random sheduler anyway (for step 1)!
        #self.agents_schedule = SchedulerFramework.RandomActivation(self) # Scheduler
        ...
        for beat_object, num_agents in configuration.items() :
            ...
            if (num_agents == 1) :
                a = AgentFramework2.Agent(agent_ID, self, idle_strategy, beat_object.centroid_node, beat_object.precinct, beat_object)
    ...
    def run_model(self,step_time, num_steps = None):
        ...
        * Inputs:
        ...
        print('END - there are {} incidents with status 0'.format(len([incident for incident in self.incidents if incident.status == 0])))
        print('END - there are {} incidents with status 1'.format(len([incident for incident in self.incidents if incident.status == 1])))
        print('END - there are {} incidents with status 2'.format(len([incident for incident in self.incidents if incident.status == 2])))
        print('END - there are {} incidents with status 3'.format(len([incident for incident in self.incidents if incident.status == 3])))
        print('END - there are {} incidents with status 4'.format(len([incident for incident in self.incidents if incident.status == 4])))
    ...
    def make_gif(self, agents_pos_all_steps, inc_pos_all_steps, ABM_NUM_STEPS, idle_strat):
        ...
        # get pecincts polygons
        patrol_beats_df = gpd.read_file('Data/DPD_Scout_Car_Areas-shp/DPD_SCAs_preprocessed.shp')

        # get all station nodes
        stations_pos = pd.read_csv('Data/Police_stations_preprocessed.csv')['Node'].tolist()
        ...
        def visualise(step, road_network, gdf_nodes, precincts, patrol_beats, agents_pos, inc_pos, stations_pos,
            ...
            """## Plot the street network and the hot edges
            liste_pairs = hot_streets.index.tolist()
            colors = ['red' if element[0] in liste_pairs else 'grey' for element in road_network.iterrows()]
            widths = [2 if element[0] in liste_pairs else 0.5 for element in road_network.iterrows()]
            road_network.plot(alpha=1, ax=ax, linewidth=widths, edgecolor=colors, zorder = 0)
            """
            ...
            #  plot stations and incidents
            #gdf_nodes.plot(ax=ax, color=colours, markersize=sizes, zorder = 2)


            # plot incidents as image 
            image_inc = OffsetImage(plt.imread('./Images/incident.png'), zoom = 0.09)
            ...
            image_car = OffsetImage(plt.imread('./Images/car.png'), zoom = 0.15)
            ...
            """ # plot stations as image
            image_station = OffsetImage(plt.imread('Images/badge2_bare.png'), zoom = 0.05)
            ax.scatter(X_stations, Y_stations, s = 0.0001, zorder=100)
            for x, y in zip(X_stations, Y_stations):
                ab = AnnotationBbox(image_station, (x, y+0.002), frameon=False)
                ab.set_zorder(5)
                ax.add_artist(ab)
             """
        ...
        """ FOR ONE SCA ONLY
        patrol_beat = 415
        sca = patrol_beats[patrol_beats['name'] == patrol_beat]
        #print("SCA number: {}" .format(sca['name']))
        poly = sca.geometry.iloc[0]
        # The patrol beat boundary
        gs = gpd.GeoSeries(poly)
        
        G_sca = ox.truncate.truncate_graph_polygon(self.graph.G, poly, retain_all=False, truncate_by_edge=False, quadrat_width=0.05, min_num=3)
        gdf_nodes_sca, gdf_edges_sca = ox.graph_to_gdfs(G_sca)

        hot_streets_global = self.dict_hot_streets_in_patrol_beats[patrol_beat]
        """

        # Convert them back to latlong
        hot_streets_latlon = hot_streets_global.to_crs("EPSG:4326")

        # truncate graph for patrol beat
        


        """images = [visualise(step, gdf_edges_sca, gdf_nodes_sca, precincts, gs, agents_pos_all_steps[step], 
                            inc_pos_all_steps[step], targets_pos_all_steps[step], stations_pos,
        ...
        # create a directory if does not already exists
        directory = "./Images/Agent_animation/{}/{}_agents".format(idle_strat, len(self.agents_schedule.agents))
        if not os.path.exists(directory):
            os.makedirs(directory)

        # save gif in said directory
        imageio.mimsave(directory+'/{}_iterrations.gif'.format(len(date_times)), images, duration = .5)
        """images[0].save(directory+'/{}_iterrations.gif'.format(len(date_times)), 
    ...
    def evaluate_model(self, fail_threshold = 15):
        ...
        num_objectives = 1: single obj optimisation
        num_objectives = 2: multi-obj optimisation"""
    ...
    def validate_model(self):
        """ To validate model, get the average of all real and simulated differences dispatch time across incidents """
        ...
        #diff_dispatch_time = np.subtract(real_dispatch_times, dispatch_times)
        #avg_diff_dispatch_time = np.mean(np.round(diff_dispatch_time))
