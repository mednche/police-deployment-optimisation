...
class Graph:
    ...
    def move_agent(self, agent, node_id):
        """ This function moves an agent entity to a new node_id. """
        agent.pos = node_id
    
    
    def findNodeWithPath(self, start_node, target, nodes_list, weight, target_is_problem = False):
        """ This function is used to check whether there is a route (due to one way roads) to the target node. 
        If there is no route, create one by creating a new reverse edge.
        - Input: 
            * start_node, 
            * target (node)
            * nodes_list
            * target_is_problem means the target node cannot be reached because it is on a one way road. 
    ...
    def findClosestHotStreetIndex(self, node, remaining_hot_streets):
        """ This function finds and returns the edge index of the closest hot street amongst those remaining to be patrolled.
            Inputs:
            - node: the node from where to find the closest hot street (can be agent.pos or the u or v node of an edge when calculating the total route in advance)
            - remaining_hot_streets: the list of hot streets yet to be patrolled along the patrol route
        """
    ...
    def getDriveTime(self, route):
        """ This function gets the driving time (sum of driving time on all edges) for a given route"""
    ...
    def findBestRoute(self, start_node, target_node, weight = 'travel_time_mins'):
        """This method calculate shortest path from start_node to a new target node and returns it
        ...
        If there is no route, create one by creating a new reverse edge.
        - Returns the found shortest route
        ...
        NB: We have to keep the complex code for case 1 and 2 in case there is no possible reverse route 
        (i.e. if oneway road in one direction near agent pos AND oneway road in another near target or the opposite. """

        origin_of_problem = 0

        # Try to find a path from the start_node (current pos of agent)
        ...
        # (path found does not go all the way to the target)
        else:
    ...
    def add_necessary_edges_at_route_ends(self, route) :
        """This function appends new edges to the the graph G (road network) to fix issues with navigating along oneway roads"""
    ...
    def get_driving_time_edge(self, u, v):
        """ This function directly looks up the driving time for a particular edge (between u and v) in the graph G (using networkx)
        - Input: u and v are two consecutive nodes on the graph
        - Returns: travel_time value in minutes for that edge """
        
        # I should use an index to look this one up! Much much faster
        # note: if multiple parrallel road between node u and v, select the first one 
        # (not ideal: I should check which one has the lowest travel time but speed of running the code matters more)
        return self.G.get_edge_data(u, v)[0]['travel_time_mins']
    
                              
    def get_length_edge(self, u, v):
        """ This function looks up the length of a particular edge (between u and v) in G directely (using networkx)
        - Input: u and v are two consecutive nodes on the graph
        - Returns: length value for that edge"""
        return self.G.get_edge_data(u, v)[0]['length']
        

    def get_edges_node(self, node) :
        """This function returns a dataframne contining the edges that surround a given node in the road network graph.
        """
    ...
    # =============================================================================
    #                                   DETROIT ONLY: density_hist_inc                           
    # =============================================================================

    
    def initAttributeDensityHistInc(self):
        """ DETROIT CASE STUDY ONLY
        This function initialise (in place) the column 'density_hist_inc' in the edge geodataframe 
        'gdf_edges' by populating it with zeros. Returns nothing.
        """
    ...
    def calculateDensityIncEdges(self, incidents):
        ...
        def calculateNumIncSingleEdge(edge, node, num_inc_node):
            """This function calculates the density_hist_inc for an edge based on the proportion of the total length of all 
    ...
    def add_density_hist_inc_desc_attribute_edges(self):
        """ This function add (in place) a new column called 'density_hist_inc_desc' to gdf_edges that transforms 'density_hist_inc' into (max_num_inc - density_hist_inc) so it can be used
    ...
    def get_streets_with_incident_in_beat(self, beat_num) :
        """ This function returns the streets with incidents in the patrol beat using the column 'patrol_beat' created in pre-processing
        for each edge in gdf_edges"""
    ...
    def get_streets_within_beat(self, patrol_beat_poly):
        """ This function returns a subset of gdf_edges (geodataframe) with edges that fall into a particular patrol beat.
        Inputs:
        - patrol_beat_poly: the geometry of the boundary of the patrol beat.
        """
        return self.gdf_edges[self.gdf_edges.intersects(patrol_beat_poly)]


    def udpate_G(self):
        """ This function refreshes (in place) the graph G from the geodataframes (gdf_nodes and gdf_edges) after they have been changed.
        """
        self.G = ox.graph_from_gdfs(self.gdf_nodes, self.gdf_edges)
