...
def run_ABMs_on_one_shift(shift, historical_crimes_scenario): 
    ...
    def run_ABMs_for_value(num_hot_streets, historical_crimes_scenario) :
        try:
            warnings.filterwarnings('ignore')
            ...
            ABM_env = Env.Environment('../', cfs_incidents_shift, ABM_START_DATETIME, ABM_END_DATETIME, num_hot_streets=num_hot_streets, historical_crimes_scenario=historical_crimes_scenario)
            ...
            df_for_num_agents.insert(0, "num_hot_streets", np.repeat(num_hot_streets, len(num_agents)))
    ...
    list_num_hot_streets = [2,5,10]

    list_of_dfs_for_value = map(partial(run_ABMs_for_value, historical_crimes_scenario=historical_crimes_scenario), list_num_hot_streets)

    warnings.filterwarnings('default')
    
    df_for_shift = pd.concat(list_of_dfs_for_value, ignore_index=True)

    #dict_for_shift = dict(zip(list_num_hot_streets,list_of_dfs))
...
def main():
    ...
    pool = Pool(cpu_count())
    list_of_shift_dfs = pool.map(partial(run_ABMs_on_one_shift, historical_crimes_scenario=historical_crimes_scenario), list_shifts)
    print('Closing Pool...')
    pool.close()
    print('Joining Pool...')
    pool.join()
    ...
    print(os.getcwd())
    with open('results_SA_hotStreets_scenario1.pkl', 'wb') as f:
        pickle.dump(df, f)
    ...
    print(os.getcwd())
    with open('results_SA_hotStreets_scenario2.pkl', 'wb') as f:
        pickle.dump(df, f)
